...
def get_dashboard(self, Dashboard):
    ...
    try:
        ...
        dashboard["project_manager"] = User.objects.filter(
            designation="Project Manager").count()
        dashboard["employee"] = User.objects.filter(
            designation="Employee").count()
        dashboard["total_projects"] = Project.objects.all().count()
        dashboard["active_projects"] = Project.objects.filter(
            status="1").count()
        proje_manager = Project.objects.filter(
            project_lead=self.request.user).count()
        employees = Project.objects.filter(
            projectteam__teammate=self.request.user).count()
        dashboard["user_total_project"] = proje_manager + employees
        project_manager = Project.objects.filter(
            project_lead=self.request.user, status=1).count()
        employee = Project.objects.filter(
            projectteam__teammate=self.request.user, status=1).count()
...
def get_piechart(user):
    ...
    if not user.is_superuser:
        project = Project.objects.filter(
            ((Q(project_lead=user) | Q(projectteam__teammate=user)))).distinct()
    else:
